...
class L1LossSum(nn.Module):
    def __init__(self,Ld_ratio=0.3):
        super().__init__()
        self.ratio = Ld_ratio
...
def calculate_depth_grad(output,h):
    dims = output.shape
    i_shift = torch.cat([output[:,:,h:,:], torch.zeros(dims[0],dims[1],dims[2]-h,dims[3])],dim=2)
    i_norm = i_shift.abs().mean()

    j_shift = torch.cat([output[:,:,:,h:], torch.zeros(dims[0],dims[1],dims[2],dims[3]-h)],dim=3)
